...
//Slide show javascript from jssor.com
...
//javascript for budget calculator
...
function budget(thisform) {
   var result = 0;
   var monthlyIncome = getRidOfBlanks(thisform.monthlyIncome.value);
   if (monthlyIncome != '' && monthlyIncome != parseFloat(monthlyIncome)) {
      alert("Monthly Income must be numeric");
      thisform.monthlyIncome.focus();
      return false;
   }
   result += Number(monthlyIncome);
   var availableBalance = getRidOfBlanks(thisform.availableBalance.value);
   if (availableBalance != '' && availableBalance != parseFloat(availableBalance)) {
      alert("Available Balance must be numeric");
      thisform.availableBalance.focus();
      return false;
   }
   result += Number(availableBalance);
   var travel = getRidOfBlanks(thisform.travel.value);
   if (travel != '' && travel != parseFloat(travel)) {
      alert("Travel must be numeric");
      thisform.travel.focus();
      return false;
   }
   result -= Number(travel);
   var sweets = getRidOfBlanks(thisform.sweets.value);
   if (sweets != '' && sweets != parseFloat(sweets)) {
      alert("Sweets must be numeric");
      thisform.sweets.focus();
      return false;
   }
   result -= Number(sweets);
   var hobbies = getRidOfBlanks(thisform.hobbies.value);
   if (hobbies != '' && hobbies != parseFloat(hobbies)) {
      alert("Hobbies must be numeric");
      thisform.hobbies.focus();
      return false;
   }
   result -= Number(hobbies);
   var other = getRidOfBlanks(thisform.other.value);
   if (other != '' && other != parseFloat(other)) {
      alert("Other must be numeric");
   ...
   var totalIncome = monthlyIncome + availableBalance;
   var totalExpenses = travel + sweets + hobbies + other;
   var total = totalIncome - totalExpenses;
   
   if (total > 0) {
	   alert("You are under budget!")
	   document.getElementById("colourChange").style.color="green";
   } else if(total < 0) {
	   alert("You are over budget!")
	   document.getElementById("colourChange").style.color="red";
   } else {
	    alert("You have broke even!");
   }
}
